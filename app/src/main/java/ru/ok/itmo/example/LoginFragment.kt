...
class LoginFragment : Fragment(R.layout.login_fragment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        button.setOnClickListener {
            ...
            if (validation(loginText, passwordText)) {
                viewModel.login(LoginData(loginText, passwordText))
            } else {
                Toast.makeText(requireActivity(), R.string.log_error, Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun validation(login: String, password: String): Boolean {
        if (login.isEmpty()) {
            return false
